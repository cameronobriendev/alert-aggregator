...
function buildPlatformQuery(platform, timeRange = 'newer_than:3m') {
  return `(${PLATFORM_PATTERNS[platform]}) ${timeRange}`
}

// Build all platforms query with optional time range
function buildAllPlatformsQuery(timeRange = 'newer_than:3m') {
  return `(${Object.values(PLATFORM_PATTERNS).map(d => `(${d})`).join(' OR ')}) ${timeRange}`
}

// Legacy exports (for backward compatibility)
export const PLATFORM_QUERIES = {
  zapier: buildPlatformQuery('zapier'),
  make: buildPlatformQuery('make'),
  airtable: buildPlatformQuery('airtable'),
  bubble: buildPlatformQuery('bubble'),
}

export const ALL_PLATFORMS_QUERY = buildAllPlatformsQuery()
...
export async function scanForUsageEmails(accessToken, platform = null, options = {}) {
  const timeRange = options.timeRange || 'newer_than:3y'
  const query = platform
    ? buildPlatformQuery(platform, timeRange)
    : buildAllPlatformsQuery(timeRange)
  ...
  console.log(`[GMAIL] Found ${messageList.length} message IDs, fetching content...`)
...
export function createGmailClient(refreshToken) {
  ...
  return {
    ...
    scanPlatform: async (platform, options = {}) => {
      const token = await ensureToken()
      return scanForUsageEmails(token, platform, options)
    },
    scanAll: async (options = {}) => {
      const token = await ensureToken()
      return scanForUsageEmails(token, null, options)
    },
  }
}
