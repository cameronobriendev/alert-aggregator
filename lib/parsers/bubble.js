...
// Domains: bubble.io (system), bubbleapps.io (app emails: [appname]-no-reply@bubbleapps.io), bubble.is (legacy)
// Uses SendGrid for email delivery
// Alert thresholds: 75% WU (warning), 100% WU (alert)
// Alert types: Usage + Errors (capacity, offline, storage, CPU, rate limits)

export function isBubbleEmail(email) {
  const from = email.from?.toLowerCase() || ''
  const body = (email.body || email.snippet || '').toLowerCase()

  // Match bubble.io, bubbleapps.io (app-generated emails), bubble.is (legacy)
  const isBubbleDomain = from.includes('bubble.io') ||
                         from.includes('bubbleapps.io') ||
                         from.includes('bubble.is')

  // Bubble also sends via SendGrid, so check for Bubble-specific content
  const isSendGridBubble = from.includes('sendgrid') &&
                           (body.includes('bubble') || body.includes('workload') || body.includes('capacity'))

  return isBubbleDomain || isSendGridBubble
}
