...
// Domains: zapier.com (+ subdomains: mail, robot, parser, interfaces), zapiermail.com (Email by Zapier action)
// ESPs: HubSpot, SendGrid, Mailgun, SparkPost
// Alert types: Usage (80%, 100%, overage) + Errors (zap failures, turned off, auth, deprecation)

export function isZapierEmail(email) {
  const from = email.from?.toLowerCase() || ''
  // Match zapier.com (and all subdomains like mail.zapier.com, robot.zapier.com)
  // Also match zapiermail.com (used for "Email by Zapier" action)
  return from.includes('zapier.com') || from.includes('zapiermail.com')
}
...
// Parse error notifications (zap failures, turned off, auth issues, deprecation, etc.)
function parseZapierError(subject, body, email) {
  const subjectLower = subject.toLowerCase()
  const bodyLower = body.toLowerCase()

  // Authentication failed - CRITICAL (automations will stop)
  if (bodyLower.includes('authentication problem') || bodyLower.includes('reconnect your account') ||
      subjectLower.includes('authentication') && subjectLower.includes('problem')) {
  ...
  // App deprecation - need to migrate
  if (bodyLower.includes('deprecation') || bodyLower.includes('need to be updated to the new version') ||
      subjectLower.includes('deprecat')) {
    const appName = extractAppName(body)
    return {
      platform: 'zapier',
      type: 'error',
      errorType: 'app_deprecation',
      severity: 'warning',
      itemName: appName,
      errorMessage: 'App integration deprecated - migration required',
      emailDate: email.date,
      emailSubject: subject,
      rawEmailId: email.id,
    }
  ...
  // Payment failed
  if (subjectLower.includes('payment') && (subjectLower.includes('failed') || bodyLower.includes('could not be processed'))) {
    return {
      platform: 'zapier',
      type: 'billing',
...
function extractAppName(body) {
  // Pattern: "'[Account Name] ([App Name])' account" or "[App] integration"
  const match = body.match(/['""]([^'""]+)\s*\(([^)]+)\)['""]/) ||
                body.match(/([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\s+integration/i) ||
                body.match(/using\s+the\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)/i)
